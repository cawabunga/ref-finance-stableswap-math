use crate::ref_finance::utils::{u128_ratio, FEE_DIVISOR};
...
impl AdminFees {
    pub fn new(admin_fee_bps: u32) -> Self {
        AdminFees {
            admin_fee_bps,
            exchange_id: "".to_string(), // env::current_account_id(),
            referral_info: None,
        }
    }
    ...
    pub fn calc_referral_share(&self, admin_share: u128) -> (u128, AccountId) {
        if let Some((referral, referral_fee)) = &self.referral_info {
            let referral_share =
                u128_ratio(admin_share, *referral_fee as u128, FEE_DIVISOR as u128);
