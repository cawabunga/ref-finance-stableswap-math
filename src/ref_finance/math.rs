...
use crate::ref_finance::admin_fee::AdminFees;
use crate::ref_finance::utils::{u128_ratio, FEE_DIVISOR, U256};
...
impl StableSwap {
    ...
    pub fn compute_y(
        ...
        c = c.checked_mul(d)?.checked_div(
            ann.checked_mul((n_coins as u128).checked_pow(n_coins as u32)?.into())?
                .into(),
        )?;
